...
export class AuthService {
  ...
  private handleValidationError(error: ZodError) {
    ...
    throw new AppError(ERROR_CODES.VALIDATION_ERROR, HttpStatusCode.BAD_REQUEST, { errors });
  }
    ...
    if (error instanceof PrismaClientKnownRequestError) {
      switch (error.code) {
        case 'P2002':
          ...
          throw new AppError(ERROR_CODES.DUPLICATE_ENTRY, HttpStatusCode.CONFLICT, { field });
        case 'P2025':
  ...
  async login(data: unknown) {
    try {
      ...
      if (!usuario) {
        throw new AppError(ERROR_CODES.INVALID_CREDENTIALS, HttpStatusCode.UNAUTHORIZED);
      }

...
describe('Payment Integration Tests', () => {
  ...
  describe('processarWebhook', () => {
    it('deve processar webhook de pagamento aprovado com sucesso', async () => {
      ...
      expect(updatedOrder?.transactionAmount).toBe(100.00);
    });
  ...
  describe('consultarPagamento', () => {
    ...
    it('deve lançar erro quando o pagamento não existe', async () => {
      ...
      await expect(
        paymentService.consultarPagamento('non-existent-payment')
      ).rejects.toThrow(
        new AppError(
  ...
  describe('reembolsarPagamento', () => {
    ...
    it('deve reembolsar valor específico com sucesso', async () => {
      ...
      await paymentService.reembolsarPagamento('test-payment-id', 50.00);

      expect(mockMercadoPago.payment.refund).toHaveBeenCalledWith('test-payment-id', { amount: 50.00 });
    });
    ...
    it('deve lançar erro quando o reembolso falha', async () => {
      ...
      await expect(
        paymentService.reembolsarPagamento('test-payment-id')
      ).rejects.toThrow(
        new AppError(
          ERROR_CODES.PAYMENT_ERROR,
          HttpStatusCode.INTERNAL_SERVER_ERROR,
